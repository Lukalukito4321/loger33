...
async function syncSlashCommands() {
  ...
  const kickCmd = new SlashCommandBuilder()
    ...
    .addBooleanOption(opt =>
      opt.setName('dry_run')
        .setDescription('If true, only shows count')
        .setRequired(false)
    );
...
// ===== interactionCreate: /dashboard + /kick =====
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.commandName === 'kick') {
    ...
    // Fetch members so role.members is accurate
    await interaction.guild.members.fetch().catch(() => null);

    const targets = role?.members;
    if (!targets || targets.size === 0) {
      return interaction.editReply(`ამ როლზე (${role?.name || 'unknown'}) წევრები ვერ მოიძებნა.`);
    }
    ...
    for (const member of targets.values()) {
      ...
      // bot hierarchy: cannot kick if member >= bot role
      if (memberTop >= botTop) {
        skipped++;
        continue;
      }
